...
export default function TranscriptUploadCard() {
  // State to store the uploaded transcript image and file name
  const [transcriptImage, setTranscriptImage] = useState<string | null>(null);
  ...
  const handleFileChange = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (files && files[0]) {
      ...
      if (file instanceof Blob) {
        ...
        // Set the transcript image with the created URL
        setTranscriptImage(imageUrl);
      } else {
